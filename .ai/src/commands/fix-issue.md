...
5. Write or update a test that would have caught this bug: a unit test in the owning module, or an integration test in `tests/<surface>.rs` using the `Project` harness.
6. Run `cargo test` to verify all tests pass, then `cargo build --release`.
