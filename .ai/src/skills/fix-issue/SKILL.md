...
2. **Reproduce** — run the failing flow against a fresh fixture (use `Project::empty()` or `Project::seeded(&[])` from `tests/common/mod.rs` so you don't contaminate the working tree). A bug you can't reproduce is a bug you can't verify fixed.
...
7. **Add a regression test** — fails on `main`, passes on the fix. A pure function gets a unit test in its module, named by behaviour; a command's output gets an integration test in `tests/<surface>.rs`: `fn sync_<what the bug broke>_stays_correct()`, hermetic temp dirs via the `Project` harness.
8. **Run the full suite** — `cargo test` (or `cargo test --test <stem>` while iterating), then `cargo build --release`. Every test green, not just the new one.
...
- The suite runs on macOS, Linux, and Windows in CI. A path assertion that passes on macOS can fail on Windows when it compares a `PathBuf` instead of the engine's `/`-separated spelling; a test that needs a POSIX-only facility is `#[cfg(unix)]` with the reason.
