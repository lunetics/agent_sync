...
- **Windows.** The first run of the native CI job on `windows-latest` (run 35424805832, 2026-09-19, on the 0.37.0 release commit) answered the question: every command that reaches the binary fails with `Error: Directory not found: .`, because under Git Bash the binary receives the suite's POSIX paths (`/tmp/…`, `$PWD`) and cannot resolve them, and after `dedupe requires TTY without --yes` the binary blocked on the runner's console (`is_tty` true, a prompt read from `CONIN$`) until the 90-minute timeout; only the Bash-unit files passed (39 cases). The step is `continue-on-error` with a 30-minute limit until a Windows follow-up slice translates MSYS paths (or the helper does, through `cygpath`) and detects the runner console; the Bash shard matrix stays the Windows gate meanwhile.
- **The cutover release.** 0.37.0 was cut on 2026-09-19 by the maintainer's installed Bash 0.36.0, which bumped `VERSION` alone; `fc98d86` carried the crate version and the changelog, the tag was moved onto it, and `release.yml` was dispatched by hand: all nine jobs green, sixteen assets published. `tests/install.bats` needed one more fix for Git Bash (`agentsync.exe` on a Windows target, `cmp` where `ln -s` copies).
