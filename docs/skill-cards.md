...
`agentsync skills` displays skill metadata from an explicitly selected,
...
```text
agentsync skills list --catalog FILE [--source ALIAS=LOCAL_REPO]... [--include GLOBS] [--exclude GLOBS]
agentsync skills show ID --catalog FILE [--source ALIAS=LOCAL_REPO]...
```
...
The pilot references its v1.1.0 skill taxonomy. These mappings are curator
judgments, not an extension to the Agent Skills frontmatter specification.
...
agentsync skills list \
  ...
  --source claude-agents=/work/claude-agents

agentsync skills show pdf \
  --catalog docs/examples/skill-cards/pilot/catalog.tsv \
  --source claude-agents=/work/claude-agents
```
