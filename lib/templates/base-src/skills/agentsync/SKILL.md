...
- `version_pin.mode` controls pin mismatches in `agent_sync.yaml`: `committed` outputs remain strict, while `local` outputs warn by default. Set `version_pin.mode: strict` to make local mismatches fatal; `warn` preserves the default. The scalar shorthand `version_pin: strict` (or `warn`) is also accepted. Unknown modes are rejected before writing.
