//! `agentsync skills`: an explicit, read-only view of experimental skill cards.
...
pub const HELP: Help = Help {
    ...
    synopsis: &[
        "skills list --catalog <file> [--source <alias=local-repo>]... [--include <globs>] [--exclude <globs>]",
        "skills show <id> --catalog <file> [--source <alias=local-repo>]...",
    ],
    description: &[
    ...
    examples: &[
        "skills list --catalog docs/examples/skill-cards/pilot/catalog.tsv",
        "skills list --catalog catalog.tsv --source claude-agents=/work/claude-agents --include 'pdf*'",
        "skills show pdf --catalog catalog.tsv --source claude-agents=/work/claude-agents",
    ],
};
...
fn parse(args: &[String]) -> Result<Args, String> {
    let action = match args.first().map(String::as_str) {
        ...
        _ => return Err("Usage: agentsync skills <list|show> --catalog <file>".to_string()),
    };
    ...
    if matches!(action, Action::Show) && id.is_none() {
        return Err("skills show requires an id".to_string());
    }
    Ok(Args {
        action,
        id,
    ...
    fn help_needs_no_catalog_or_source() {
        ...
        assert!(
            String::from_utf8(out)
                .unwrap()
                .contains("agentsync skills list")
        );
        assert!(err.is_empty());
    }
