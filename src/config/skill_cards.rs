...
/// Read the whole explicit catalog before returning any card. This is not a
/// general TSV parser: its small grammar keeps catalog text inert and makes
/// every validation decision visible at this boundary.
...
/// The pilot stores only path spelling here. It deliberately does not claim
/// that an OASF taxonomy path still exists or means the same thing upstream.
