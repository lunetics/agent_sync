...
/// A narrow ASCII path grammar keeps revision/path syntax out of Git arguments.
...
pub fn read(
    ...
    // Do not silently discover a repository above an incorrectly mapped folder.
...
// yaml_subset deliberately preserves the engine's permissive Bash semantics
// (first duplicate wins, literal escapes). A source declaration needs an honest
// unsupported state instead; do not change the shared config reader for cards.
...
mod tests {
    ...
    fn pinned_symlinks_are_not_followed() {
        let (repo, _) = fixture();
        // The removed file is owned by this temporary fixture, not a checkout.
