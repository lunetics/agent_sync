...
@test "pilot catalog has sourced requirements for every selectable variant" {
    run python3 "$REPO_ROOT/tests/check_mcp_catalog.py" --as-of 2026-09-16 --fail-stale
    [ "$status" -eq 0 ]
    [[ "$output" == *'"id": "octocode"'* ]]
    [[ "$output" == *'"id": "context7"'* ]]
...
@test "requirements sweep reports stale metadata without mutating catalog" {
    ...
    run python3 "$REPO_ROOT/tests/check_mcp_catalog.py" --catalog "$TEST_PROJECT/catalog" --as-of 2026-12-01 --fail-stale
    [ "$status" -eq 1 ]
    [[ "$output" == *'"status": "due"'* ]]
    [ "$before" = "$(file_sha256 "$TEST_PROJECT/catalog/octocode/manifest.json")" ]
}
...
@test "requirements metadata rejects credential values" {
    ...
    run python3 "$REPO_ROOT/tests/check_mcp_catalog.py" --catalog "$TEST_PROJECT/catalog" --as-of 2026-09-16
    [ "$status" -eq 2 ]
